...
CREATE TABLE IF NOT EXISTS console_user (
    ...
    role         INT          NOT NULL DEFAULT 4 CHECK (role >= 1), -- unused(0), admin(1), developer(2), maintainer(3), readonly(4)
    update_time  TIMESTAMPTZ  NOT NULL DEFAULT now(),
    username     VARCHAR(128) NOT NULL
);
CREATE UNIQUE INDEX IF NOT EXISTS console_user_email_uniq ON console_user (email);
CREATE UNIQUE INDEX IF NOT EXISTS console_user_username_uniq ON console_user (username);
