...
const (
	...
	Nakama_Realtime_FullMethodName                          = "/nakama.api.Nakama/Realtime"
)
...
type NakamaClient interface {
	...
	// Open a realtime bidirectional stream using the same Envelope protocol as the WebSocket endpoint.
	Realtime(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[rtapi.Envelope, rtapi.Envelope], error)
}
...
func (c *nakamaClient) Realtime(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[rtapi.Envelope, rtapi.Envelope], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Nakama_ServiceDesc.Streams[0], Nakama_Realtime_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[rtapi.Envelope, rtapi.Envelope]{ClientStream: stream}
	return x, nil
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Nakama_RealtimeClient = grpc.BidiStreamingClient[rtapi.Envelope, rtapi.Envelope]
...
type NakamaServer interface {
	...
	// Open a realtime bidirectional stream using the same Envelope protocol as the WebSocket endpoint.
	Realtime(grpc.BidiStreamingServer[rtapi.Envelope, rtapi.Envelope]) error
...
func (UnimplementedNakamaServer) Realtime(grpc.BidiStreamingServer[rtapi.Envelope, rtapi.Envelope]) error {
	return status.Error(codes.Unimplemented, "method Realtime not implemented")
}
...
func _Nakama_Realtime_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(NakamaServer).Realtime(&grpc.GenericServerStream[rtapi.Envelope, rtapi.Envelope]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Nakama_RealtimeServer = grpc.BidiStreamingServer[rtapi.Envelope, rtapi.Envelope]
...
var Nakama_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "Realtime",
			Handler:       _Nakama_Realtime_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
