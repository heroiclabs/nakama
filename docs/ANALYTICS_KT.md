...
These fields are lifted from the top-level payload into `eventData` automatically:
...
| `analytics_funnel` | analytics_extended.js | Acquisition to activation to monetization funnel |
...
| `analytics_feature_adoption` | analytics_extended.js | Percentage of users who have used each feature |
...
| `analytics_conversion_funnel` | analytics_extended.js | Paywall shown to IAP started to purchased funnel |
...
| `analytics_modes_transitions` | analytics_modes.js | Mode to mode transition matrix |
...
| RPC ID | File | Description |
|---|---|---|
| `satori_identity_sync` | analytics_satori_identity.js | Compute 10 traits for one user and push to Satori |
...
| `satori_segments_winback` | analytics_segments.js | Fire winback_eligible for inactive users |
...
These functions exist in `analytics_v2.js` but are **not** currently registered via `registerRpc`. Pending migration:
...
| Collection | Key Pattern | Description |
|---|---|---|
| `analytics_events` | `dash_{gameId}_{YYYY-MM-DD}_{eventName}_{unixTs}_{rand6}` | Every accepted event. Primary source for rollup scanners. |
| `analytics_failed_events` | `fail_{YYYY-MM-DD}_{unixTs}_{rand6}` | Rejected event ring buffer (dashboard Pipeline tab). |

### 5.2 Engagement Counters

| Collection | Key Pattern | Description |
|---|---|---|
| `analytics_dau` | `dau_{gameId}_{YYYY-MM-DD}` | Per-game DAU list (uniqueUsers[] + count + newUsers) |
| `analytics_dau` | `dau_platform_{YYYY-MM-DD}` | Cross-game platform-wide DAU |
| `analytics_sessions` | `session_stats_{YYYY-MM-DD}` | Platform-wide session aggregates |
| `analytics_sessions` | `session_stats_{gameId}_{YYYY-MM-DD}` | Per-game session aggregates |
| `analytics_platform` | `platform_{gameId}_{YYYY-MM-DD}_{platform}` | Per-platform event count |
| `analytics_user_first_seen` | `first_{userId}_{gameId}` | Atomic first-seen marker (create-once with version:"*") |
| `analytics_live_daily` | `live_{gameId}_{YYYY-MM-DD}` | Real-time OCC counter (total, by_name, by_platform, by_country, revenue_usd) |
| `analytics_live_daily` | `live_all_{YYYY-MM-DD}` | Real-time cross-game aggregate |
| `analytics_metrics_counters` | `counter_{YYYY-MM-DD}` | events_accepted, events_rejected, log_calls, schema_v2_*, satori_* counters |

### 5.3 Rollup Docs (Written by Nightly Cron)

| Collection | Key Pattern | Description |
|---|---|---|
| `analytics_rollup_daily` | `rollup_{gameId}_{YYYY-MM-DD}` | Pre-aggregated daily KPIs per game |
| `analytics_rollup_daily` | `rollup_all_{YYYY-MM-DD}` | Pre-aggregated cross-game daily KPIs |
| `analytics_retention` | `cohort_{gameId}_{YYYY-MM-DD}` | D1/D3/D7/D14/D30 retention per cohort-date |
| `analytics_funnel_daily` | `funnel_{gameId}_{YYYY-MM-DD}` | Daily funnel step counts |
| `analytics_rollup_meta` | `last_success` | Rollup run bookkeeping |
| `analytics_rollup_meta` | `platform_totals` | total_users_lifetime (cumulative) |

### 5.4 Phase-4 Pre-Aggregated Tabs

| Collection | Key Pattern | Description |
|---|---|---|
| `analytics_modes_daily` | `modes_{gameId}_{YYYY-MM-DD}` | Per-quiz-mode KPIs (sessions, users, completes, abandons, revenue, avg score) |
| `analytics_dropoff_daily` | `dropoff_{gameId}_{YYYY-MM-DD}` | Churn signal counts + per-question abandonment + per-screen exit rate |
| `analytics_question_daily` | `question_{gameId}_{YYYY-MM-DD}` | Per-question accuracy/difficulty/response time |
| `analytics_offer_daily` | `offer_{gameId}_{YYYY-MM-DD}` | IAP offer impression/conversion/revenue |

### 5.5 Player Profile (GPA)

| Collection | Key | Description |
|---|---|---|
| `game_player_analytics` | `{gameId}:{userId}` | Full per-player per-game analytics doc (owner: userId) |

GPA document top-level fields:
...
| Collection | Key | Description |
|---|---|---|
| `analytics_segments_state` | `{userId}` | Per-user segment cooldown (winback_last_sent_utc, preiap_last_sent_utc) |
| `analytics_segments_meta` | `last_run` | Last segment run summary |
| `external_analytics` | `apple_quizverse_latest` | App Store Connect snapshot |
| `external_analytics` | `play_quizverse_latest` | Google Play Console snapshot |

### 5.7 Admin

| Collection | Key | Description |
|---|---|---|
| `admin_users` | `session_{token}` | Admin session tokens (TTL: 12h) |

### 5.8 Error Tracking (v2)

| Collection | Key Pattern | Description |
|---|---|---|
| `analytics_error_events` | `error_{rpcName}_{unixTs}_{rand4}` | Structured error records (rpc_name, error_message, stack_trace, date) |
...
## 8. Event Name Aliases (Legacy to Canonical)

Applied at ingestion in `normalizeInboundEvent()` and at rollup scan time in `analytics_rollup.js`. Events are stored with the CANONICAL name after normalization.
...
**Trigger:** `analytics_rollup_run` RPC:
```json
...
6. Writes all output docs (see sections 5.3 and 5.4)
...
**Auto-assignment:** `tpGetPrivacyTier(eventName)` from `analytics_tracking_plan.js` assigns tier when client does not provide one.
...
- **URL:** `/analytics.html` (served by Nakama HTTP port or reverse proxy)
- **Login:** `admin_login` RPC yields bearer token OR use `dashboard_secret` shared secret

### Ops Counters (`analytics_metrics_counters/counter_{date}`)
...
| 5 | **OCC race on live counters** | Concurrent `iap_purchased` + `ad_impression` for same gameId/day caused clobbering. Fixed: `_liveCountersUpsert` retries up to 2x with version-check. | Fixed |
...
4. **Schema v2 fields** (`client_event_id`, `event_time`, `session_id`) are auto-injected by `AnalyticsManager.InjectGlobalContext` - do not set them manually per event
5. **Batch events** - the Unity client batches automatically; do not call the RPC directly per-event
6. **Storage key format is ISO date** (`YYYY-MM-DD`) - NEVER use unix-seconds as the date slot in analytics keys

---

*Generated from live code on 2026-06-01. For corrections, update this file and ping the backend team.*
