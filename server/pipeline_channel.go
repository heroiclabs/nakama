...
VALUES ($1, $2, $3, $4, $5, $6::UUID, $7::UUID, $8, $9, $10, $10)`
		_, err := p.db.Exec(query, message.MessageId, message.Code.Value, message.SenderId, message.Username, streamConversionResult.Stream.Mode, streamConversionResult.Stream.Subject, streamConversionResult.Stream.Descriptor, streamConversionResult.Stream.Label, message.Content, pq.FormatTimestamp(time.Unix(message.CreateTime.Seconds, 0).UTC()))
		if err != nil {
...
func (p *Pipeline) channelMessageUpdate(logger *zap.Logger, session Session, envelope *rtapi.Envelope) {
	...
	if meta.Persistence {
		...
		err := p.db.QueryRow(query, incoming.MessageId, message.SenderId, message.Content, message.Username, pq.FormatTimestamp(time.Unix(message.UpdateTime.Seconds, 0).UTC())).Scan(&dbCreateTime)
		if err != nil {
