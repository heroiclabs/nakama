...
func (s *ApiServer) ValidatePurchasePlaystation(ctx context.Context, in *api.ValidatePurchasePlaystationRequest) (*api.ValidatePurchaseResponse, error) {
	...
	playstationConfig := s.config.GetIAP().Playstation
	// change xbox token to ps token
	validation, err := s.runtime.iapPlaystationManager.PurchaseValidate(ctx, s.logger, s.db, playstationConfig.Token, playstationConfig.Environment, "", in.EntitlementLabel, userID, persist)
	if err != nil {
		return nil, err
	}
