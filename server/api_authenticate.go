...
func (s *ApiServer) AuthenticateItch(ctx context.Context, in *api.AuthenticateItchRequest) (*api.Session, error) {
	// Before hook.
	if fn := s.runtime.BeforeAuthenticateItch(); fn != nil {
		beforeFn := func(clientIP, clientPort string) error {
			result, err, code := fn(ctx, s.logger, "", "", nil, 0, clientIP, clientPort, in)
			if err != nil {
				return status.Error(code, err.Error())
	...
	if in.Account == nil || in.Account.Token == "" {
		return nil, status.Error(codes.InvalidArgument, "Itch access token is required.")
	}
	...
	dbUserID, dbUsername, created, err := AuthenticateItch(ctx, s.logger, s.db, s.socialClient, in.Account.Token, username, create)
	if err != nil {
		return nil, err
	}
	...
	// After hook.
	if fn := s.runtime.AfterAuthenticateItch(); fn != nil {
		afterFn := func(clientIP, clientPort string) error {
			return fn(ctx, s.logger, dbUserID, dbUsername, in.Account.Vars, exp, clientIP, clientPort, session, in)
		}
