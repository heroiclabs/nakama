...
type subscriptionspurchasesListCursor struct {
...
func ListPurchases(ctx context.Context, logger *zap.Logger, db *sql.DB, userID string, limit int, cursor string) (*api.PurchaseList, error) {
	var incomingCursor *subscriptionspurchasesListCursor
	if cursor != "" {
		cb, err := base64.URLEncoding.DecodeString(cursor)
		if err != nil {
			return nil, ErrPurchasesListInvalidCursor
		}
		incomingCursor = &subscriptionspurchasesListCursor{}
	...
	var nextCursor *subscriptionspurchasesListCursor
	var prevCursor *subscriptionspurchasesListCursor
	...
	for rows.Next() {
		...
		if len(purchases) >= limit {
			nextCursor = &subscriptionspurchasesListCursor{
				TransactionId: transactionId,
				PurchaseTime:  timestamppb.New(purchaseTime.Time),
		...
		if incomingCursor != nil && prevCursor == nil {
			prevCursor = &subscriptionspurchasesListCursor{
				TransactionId: transactionId,
				PurchaseTime:  timestamppb.New(purchaseTime.Time),
				UserId:        userID.String(),
				IsNext:        false,
