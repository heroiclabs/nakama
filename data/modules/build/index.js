...
var rpcAddUserGems = function (ctx, logger, nk) {
    ...
    var objectList = [];
    try {
        objectList = nk.storageRead([{ key: "array", collection: "test", userId: ctx.userId }]);
    }
    catch (error) {
        logger.error("rpcUserBuy: storageRead error: %s", error);
        throw error;
    }
    logger.debug('Object List: %s', JSON.stringify(objectList));
    return updateString;
};
...
function afterAuthenticate(ctx, logger, nk, data) {
    ...
    try {
        nk.storageWrite([writeStats, writeAddFriendQuest, writeCards, writeTest]);
    }
    catch (error) {
