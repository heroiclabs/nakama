...
func TestSatoriClient_EventsPublish(t *testing.T) {
	...
	client := NewSatoriClient(ctx, logger, "<URL>", "<API KEY NAME>", "<API KEY>", "<SIGNING KEY>", 0, 2, false)
...
func TestSatoriClientMemory(t *testing.T) {
	...
	logger := NewConsoleLogger(os.Stdout, true)
	ctx := context.Background()
	client := NewSatoriClient(ctx, logger, "<SATORI_URL>", "<SATORI_API_KEY_NAME", "<SATORI_API_KEY>", "<SIGNING_KEY>", 3600, 2, true)
