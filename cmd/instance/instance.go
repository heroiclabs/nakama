...
import (
	...
	nakamaServer "github.com/heroiclabs/nakama/v3/server"
)

type Instance struct {
	functions []func(ctx context.Context, logger nakamaRuntime.Logger, db *sql.DB, module *nakamaServer.RuntimeGoNakamaModule, initializer *nakamaServer.RuntimeGoInitializer) error
}
...
func (i *Instance) AddGoModule(target func(ctx context.Context, logger nakamaRuntime.Logger, db *sql.DB, module *nakamaServer.RuntimeGoNakamaModule, initializer *nakamaServer.RuntimeGoInitializer) error) {
	i.functions = append(i.functions, target)
}
...
func (i *Instance) boot() {
	...
	if len(os.Args) > 1 {
		...
		case "check":
			...
			flags.StringVar(&runtimePath, "runtime.path", filepath.Join(config.GetDataDir(), "modules"), "Path for the instance to scan for Lua and Go library files.")
	...
	runtime, runtimeInfo, err := server.NewRuntime(ctx, logger, startupLogger, db, jsonpbMarshaler, jsonpbUnmarshaler, config, version, socialClient, leaderboardCache, leaderboardRankCache, leaderboardScheduler, sessionRegistry, sessionCache, statusRegistry, matchRegistry, tracker, metrics, streamManager, router, storageIndex, fmCallbackHandler, i.functions)
	if err != nil {
	...
	// Gracefully stop remaining instance components.
	apiServer.Stop()
	consoleServer.Stop()
	matchmaker.Stop()
...
//
// You can disable the telemetry completely before instance start by setting the
...
// * Amount of time the instance ran for.
//
// This information is sent via Segment which allows the Nakama team to
// analyze usage patterns and errors in order to help improve the instance.
